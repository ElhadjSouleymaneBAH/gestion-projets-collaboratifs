<template>
  <div class="dashboard-container">
    ...
    <div
      v-if="!chargementGlobal && abonnement && !abonnementActif"
      class="alert alert-warning alert-compact mb-3"
    >
      <div class="d-flex justify-content-between align-items-center">
        <div>
    ...
    <div
      v-if="erreurBackend"
      class="alert alert-danger d-flex justify-content-between align-items-center mb-3"
    >
      ...
      <div class="kpi-bar mb-3">
        <div class="kpi-item" @click="onglet = 'projets'" :class="{ active: onglet === 'projets' }">
          ...
          <div class="kpi-content">
            ...
            <span class="kpi-sub text-primary"
              >{{ projetsActifs.length }} {{ $t('commun.actifs') }}</span
            >
          </div>
        </div>
        ...
        <div class="kpi-item" @click="onglet = 'kanban'" :class="{ active: onglet === 'kanban' }">
          ...
          <div class="kpi-content">
            ...
            <span class="kpi-sub text-warning"
              >{{ totalTaches.length }} {{ $t('commun.total') }}</span
            >
          </div>
        </div>
        ...
        >
          ...
          <div class="kpi-content">
            ...
            <span class="kpi-sub text-info"
              >{{ notifications.length }} {{ $t('commun.total') }}</span
            >
          </div>
        </div>
      </div>
      ...
      <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
        <ul class="nav nav-pills nav-pills-custom">
          ...
          <li class="nav-item">
            <a
              class="nav-link"
              :class="{ active: onglet === 'statistiques' }"
              @click="onglet = 'statistiques'"
              href="javascript:void(0)"
            >
      ...
      <div v-if="onglet === 'projets'">
        <div class="section-header mb-3">
          <div>
            ...
            <p class="section-subtitle">
              {{ mesProjets.length }} {{ $t('projets.projets') }} • {{ projetsActifs.length }}
              {{ $t('commun.actifs') }}
            </p>
          </div>
        ...
        <div v-else class="projects-grid">
          <div
            ...
            :key="p.id"
            class="project-card"
            :style="{ '--project-color': getProjetCouleur(p) }"
          >
            ...
            <div class="project-card-body">
              ...
              <p class="project-description">
                {{ translateProjectDescription(p.description).substring(0, 80)
                }}{{ p.description && p.description.length > 80 ? '...' : '' }}
              </p>

              <div class="project-meta">
                <span class="meta-item"
                  ><i class="fas fa-users"></i> {{ getMembresProjet(p.id).length }}</span
                >
                <span class="meta-item"
                  ><i class="fas fa-tasks"></i> {{ getTachesProjet(p.id).length }}</span
                >
                <span class="meta-item"
                  ><i class="fas fa-calendar"></i>
                  {{ formatDate(p.date_creation || p.dateCreation) }}</span
                >
              </div>
      ...
      <div v-if="onglet === 'kanban'">
        ...
        <div v-else>
          <div class="section-header mb-3">
            <div class="kanban-project-selector">
              <label class="form-label mb-1"
                ...
                >{{ $t('projets.selectionnerProjet') }}</label
              >
              <select
                class="form-select"
          ...
          <KanbanBoard
            ...
            :peut-assigner="true"
            :key="projetKanbanSelectionne.id + '-' + kanbanRefreshKey"
            @tache-assignee="chargerTaches"
          />
      ...
      <div v-if="onglet === 'equipe'">
        <div class="section-header mb-3">
          <div>
            ...
            <p class="section-subtitle">
              {{ totalMembres }} {{ $t('equipe.collaborateursSurTousProjets') }}
            </p>
          </div>
          <button
            class="btn btn-success"
          ...
          <div class="chat-sidebar">
            ...
            <div class="chat-project-list">
              ...
              >
                ...
                <span v-if="getMessagesNonLusProjet(p.id) > 0" class="badge bg-success">{{
                  getMessagesNonLusProjet(p.id)
                }}</span>
              </button>
      ...
      <div v-if="onglet === 'notifications'">
        <div class="section-header mb-3">
          <div>
            ...
            <p class="section-subtitle">
              {{ notificationsNonLues.length }} {{ $t('notifications.nonLues') }}
              {{ $t('commun.sur') }} {{ notifications.length }}
            </p>
          </div>
          <button
      ...
      <div class="section-header mb-3">
        <div>
          <h5 class="section-title">
            <i class="fas fa-chart-pie me-2 text-warning"></i>{{ $t('statistiques.titre') }}
          </h5>
          ...
          <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white">
              <h6 class="mb-0">
                ...
                >{{ $t('statistiques.repartitionTaches') }}
              </h6>
            </div>
            <div class="card-body">
            ...
            <div class="card-footer bg-light">
              <div class="d-flex justify-content-around text-center small">
                <div>
                  <span class="badge bg-secondary">{{ statsCalculees.taches.brouillon }}</span>
                  {{ $t('taches.statuts.brouillon') }}
                </div>
                <div>
                  <span class="badge bg-warning text-dark">{{
                    statsCalculees.taches.enAttente
                  }}</span>
                  {{ $t('taches.statuts.enAttente') }}
                </div>
                <div>
                  <span class="badge bg-success">{{ statsCalculees.taches.termine }}</span>
                  {{ $t('taches.statuts.termine') }}
                </div>
                <div>
                  <span class="badge bg-danger">{{ statsCalculees.taches.annule }}</span>
          ...
          <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white">
              <h6 class="mb-0">
                ...
                >{{ $t('statistiques.repartitionProjets') }}
              </h6>
            </div>
            <div class="card-body">
            ...
            <div class="card-footer bg-light">
              <div class="d-flex justify-content-around text-center small">
                <div>
                  <span class="badge bg-success">{{ statsCalculees.projets.actif }}</span>
                  {{ $t('statuts.ACTIF') }}
                </div>
                <div>
                  <span class="badge bg-warning text-dark">{{
                    statsCalculees.projets.suspendu
                  }}</span>
                  {{ $t('statuts.SUSPENDU') }}
                </div>
                <div>
                  <span class="badge bg-secondary">{{ statsCalculees.projets.termine }}</span>
                  {{ $t('statuts.TERMINE') }}
                </div>
                <div>
                  <span class="badge bg-danger">{{ statsCalculees.projets.annule }}</span>
                  {{ $t('statuts.ANNULE') }}
                </div>
              </div>
            </div>
                  ...
                  <div class="stat-box text-center p-3 bg-light rounded">
                    ...
                    <div class="progress mt-2" style="height: 6px">
                      <div
                        ...
                        :style="{ width: statsCalculees.tauxCompletion + '%' }"
                      ></div>
                    </div>
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="stat-box text-center p-3 bg-light rounded">
                    ...
                    <small class="text-muted"
                      >{{ $t('commun.sur') }} {{ totalTaches.length }}
                      {{ $t('commun.total') }}</small
                    >
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="stat-box text-center p-3 bg-light rounded">
                    <div class="stat-value text-warning">
                      {{ statsCalculees.tachesNonAssignees }}
                    </div>
    ...
    <div v-if="abonnementActif && !chargementGlobal" class="support-premium-bar">
      <div class="d-flex align-items-center gap-2">
        ...
        <span
          ...
          {{ $t('tableauBord.chefProjet.supportPremium.description') }}</span
        >
      </div>
              ...
              <div class="form-check form-switch">
                ...
                <label class="form-check-label" for="projetPublique">
                  <span v-if="projetForm.publique"
                    ><i class="fas fa-globe text-success me-1"></i>{{ $t('projets.public') }}</span
                  >
                  <span v-else
                    ><i class="fas fa-lock text-secondary me-1"></i>{{ $t('projets.prive') }}</span
                  >
                </label>
              </div>
              <small class="text-muted">{{
                projetForm.publique
                  ? $t('projets.publicDescription')
                  : $t('projets.priveDescription')
              }}</small>
            </div>
          </div>
        ...
        <div class="modal-content">
          <div class="modal-header bg-warning">
            <h5 class="modal-title">
              <i class="fas fa-edit me-2"></i>{{ $t('projets.modifierProjet') }}
            </h5>
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <input
                class="form-control"
                v-model.trim="projetModifForm.titre"
                maxlength="120"
                required
              />
            </div>
            <div class="mb-3">
              ...
              <textarea
                class="form-control"
                v-model.trim="projetModifForm.description"
                rows="3"
                maxlength="500"
              ></textarea>
            </div>
            ...
            <div class="mb-3">
              ...
              <div class="form-check form-switch">
                <input
                  class="form-check-input"
                  type="checkbox"
                  id="projetModifPublique"
                  v-model="projetModifForm.publique"
                />
                <label class="form-check-label" for="projetModifPublique">
                  <span v-if="projetModifForm.publique"
                    ><i class="fas fa-globe text-success me-1"></i>{{ $t('projets.public') }}</span
                  >
                  <span v-else
                    ><i class="fas fa-lock text-secondary me-1"></i>{{ $t('projets.prive') }}</span
          ...
          <div class="modal-footer">
            <button class="btn btn-outline-secondary" @click="fermerModalModificationProjet">
              {{ $t('commun.annuler') }}
            </button>
            <button
              class="btn btn-warning"
              @click="sauvegarderModificationProjet"
              :disabled="!projetModifForm.titre.trim()"
            >
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button class="btn btn-secondary" @click="modalCreationTache = false">
              {{ $t('commun.annuler') }}
            </button>
            <button
              class="btn btn-success"
              @click="creerNouvelleTache"
              :disabled="!nouvelleTache.titre || !nouvelleTache.projetId"
            >
              <i class="fas fa-check me-1"></i>{{ $t('commun.creer') }}
            </button>
          </div>
            ...
            <div v-else>
              <div class="mb-4">
                <label class="form-label fw-bold"
                  ><i class="fas fa-tasks me-2 text-warning"></i>1.
                  {{ $t('taches.selectionnerTache') }}</label
                >
              ...
              <div v-if="tacheAAssigner" class="mb-3">
                <label class="form-label fw-bold"
                  ><i class="fas fa-user me-2 text-success"></i>2.
                  {{ $t('taches.selectionnerMembre') }}</label
                >
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header bg-primary text-white">
            <h5 class="modal-title">
              <i class="fas fa-comments me-2"></i>{{ $t('commentaires.commentairesTache') }}
            </h5>
          ...
          <div class="modal-body">
            ...
            <div class="border-top pt-3">
              <label class="form-label fw-bold"
                ...
                >{{ $t('commentaires.ajouterCommentaire') }}</label
              >
              <textarea
...
.kanban-project-selector .form-label {
  font-size: 0.85rem;
  color: #666;
  font-weight: 500;
}
